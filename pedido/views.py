...
def finalizar_pedido(request, room_name='pedir'):
    if request.method == "GET":
        ...
        if len(request.session['carrinho']) > 0:
            ...
            total = sum([float(i['preco']) for i in request.session['carrinho']])
            if x['troco_para']:
                if int(x['troco_para']) < total:
                    messages.add_message(request, constants.ERROR, 'O valor digitado é menor que o total da compra.')
            ...
            messages.add_message(request, constants.SUCCESS, 'Obrigado pela compra. entraremos em contato para desenvolvermos o seu produto.')
            return render(request, 'pedido_realizado.html')
        else:
            messages.add_message(request, constants.ERROR, 'Escolha ao menos 1 produto.')
            return redirect('finalizar_pedido')
